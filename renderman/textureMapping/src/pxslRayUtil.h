...
float
pxslGetRaySamplesSimple(uniform float samples, factor) {
    uniform float depth;
    rayinfo("depth", depth);
    if( depth > 0 )
    {
        return  max(1,floor(pow(factor, depth) * samples + .5));
    }
    return samples;
}
...
pxslUtilShadingNormal(normal n;)
{
    ...
    if (raydepth > 0) {
        ...
        uniform float shading_normalmode = 1;//surface normal
        option("user:shading_normalmode",shading_normalmode);
        uniform float sides = 2;
        attribute("Sides", sides);

        if(shading_normalmode == 0 && sides == 2) {
            Ns = faceforward(Ns, I, Ns);
        }
    }
    return Ns;
